...
async function runPredict() {
  ...
  if (summaryBox) {
    summaryBox.innerHTML = renderPredictLoadingState();
  }

  if (metaBox) {
    metaBox.innerHTML = `<div class="empty-line">Recalculando ritmo, estrategia y favoritos…</div>`;
  }
  ...
  try {
    ...
    if (summaryBox) {
      summaryBox.innerHTML = renderPredictSummaryCards(data);
    }

    if (metaBox) {
      metaBox.innerHTML = renderPredictMetaCards(data);
    }
    ...
    if (historyBox) {
      historyBox.innerHTML = renderPredictionHistory();
    }
  } catch (error) {
    if (output) {
    ...
    if (summaryBox) {
      summaryBox.innerHTML = `<div class="empty-line">No se ha podido generar el resumen predictivo.</div>`;
    }
...
function shouldAutoGeneratePredict(favorite, raceName) {
  ...
  return (
    lastPredictContext.raceName !== raceName ||
    lastPredictContext.favoriteKey !== `${favorite.type}:${favorite.name}`
  );
}
