...
function setActiveNav(tabId) {
  ["nav-home", "nav-predict", "nav-favorito", "nav-news", "nav-more"].forEach(id => {
...
function getSettings() {
  const parsed = safeJsonParse(localStorage.getItem("racecontrolSettings"), DEFAULT_SETTINGS);
...
function getFavorite() {
  const parsed = safeJsonParse(localStorage.getItem("racecontrolFavorite"), DEFAULT_FAVORITE);
  return parsed || DEFAULT_FAVORITE;
}
...
function getTrendInfo(teamName, favorite) {
  ...
  if (score >= 2) {
    return {
      label: "Al alza",
      className: "up",
      description: "Llega con señales positivas y mejor lectura del fin de semana."
    };
  }
  ...
  if (score <= -1.5) {
    return {
      label: "A la baja",
      className: "down",
      description: "Sigue necesitando un salto claro para estabilizar el rendimiento."
    };
  }
  ...
  return {
    label: "Estable",
    className: "neutral",
    description: "Rendimiento bastante estable, sin un giro claro todavía."
  };
}
...
function getFavoriteMetrics(favorite) {
  ...
  const expectedWindow = getFavoriteStrengthWindow(favorite, teamData);
  ...
  const pointsProbability = Math.round(
    Math.max(12, Math.min(92, (teamData.racePace - 50) * 1.8 + formBoost))
  );

  const dnfRisk = Math.round(
    Math.max(8, Math.min(45, 34 - teamData.reliability * 0.27 + (100 - teamData.racePace) * 0.06))
  );

  return { teamData, trendInfo, expectedWindow, pointsProbability, dnfRisk };
}
...
async function fetchStandingsData(force = false) {
  ...
  if (!response.ok) {
    throw new Error(data?.error || "No se pudo cargar la clasificación");
  }
...
async function fetchNewsDataForFavorite(favorite, force = false) {
  ...
  if (state.newsCache[cacheKey] && !force) {
    return state.newsCache[cacheKey];
  }
...
async function fetchPredictData(favorite, raceName) {
  ...
  if (!response.ok) {
    throw new Error(data?.message || data?.error || "Error al generar la predicción");
  }
...
function getPredictionHistory() {
  const parsed = safeJsonParse(localStorage.getItem("racecontrolPredictionHistory"), []);
...
function pushPredictionHistory(data, favorite, raceName) {
  ...
  savePredictionHistory([entry, ...history].slice(0, 8));
}

function clearPredictionHistory() {
  localStorage.setItem("racecontrolPredictionHistory", JSON.stringify([]));
...
function renderPredictionHistory() {
  ...
  if (!Array.isArray(history) || history.length === 0) {
    return `<div class="empty-line">Todavía no hay predicciones guardadas.</div>`;
  }
...
function renderErrorCard(title, subtitle, message) {
  return `
    <div class="card">
      <div class="card-title">${title}</div>
      <div class="card-sub">${subtitle}</div>
      <pre class="ai-output">${escapeHtml(message)}</pre>
    </div>
  `;
}
...
function showPredict() {
  try {
    ...
    contentEl().innerHTML = `
      ...
      <div class="card">
        ...
        <pre id="predictOutput" class="ai-output">${
          !needFreshPredict
            ? formatPredictResponse(state.lastPredictData)
    ...
    contentEl().innerHTML = renderErrorCard(
      "Predicción",
      "La pestaña ha fallado al renderizar.",
      error.message
    );
  }
}
...
function buildNewsFilterPresets() {
  ...
  return [
    ...
    { key: "alonso", label: "Alonso", favoritePayload: DEFAULT_FAVORITE },
...
function renderNewsFilters() {
  ...
  return `
    <div class="filters-row">
      ${filters.map(filter => `
        <button class="chip ${state.currentNewsFilterKey === filter.key ? "active" : ""}" onclick="switchNewsFilter('${filter.key}')">${filter.label}</button>
      `).join("")}
    </div>
  `;
}
...
async function refreshCurrentNews() {
  ...
  if (!filter) return;
  delete state.newsCache[getNewsCacheKey(filter.favoritePayload)];
  showNews();
}
...
function driverRow(driver) {
  const favorite = getFavorite();
  const isFav = favorite.type === "driver" && favorite.name === driver.name;
  const isTeammate = favorite.type === "driver" && favorite.team === driver.team && favorite.name !== driver.name;
  const rowClass = [driver.pos === 1 ? "leader" : "", isFav ? "favorite-row" : "", isTeammate ? "teammate-row" : ""].join(" ").trim();

  return `
    <div class="standing-row ${rowClass}" onclick="openDriverDetail(${JSON.stringify(driver.name)}, ${JSON.stringify(driver.team)}, ${JSON.stringify(driver.number)}, ${JSON.stringify(String(driver.points))}, ${JSON.stringify(driver.colorClass)}, ${JSON.stringify(driver.pos)})">
      <div class="row-left">
        <div class="row-pos-wrap">
          <div class="row-pos">${driver.pos}</div>
          ${renderDeltaBadge(state.standingsDelta.drivers?.[driver.name] || 0)}
        </div>
        <img class="row-avatar" src="${driver.image}" alt="${driver.name}" onerror="this.style.display='none'">
        <div class="row-stripe ${driver.colorClass}"></div>
        <div class="row-number" style="color: var(--${driver.colorClass});">${driver.number}</div>
        <div class="row-info">
          <div class="row-name">${driver.name}</div>
          <div class="row-team">
            <span>${driver.team}</span>
            <img class="team-logo" src="${getTeamLogo(driver.team)}" alt="${driver.team}" onerror="this.style.display='none'">
          </div>
        </div>
      </div>
      <div class="row-badges">
        ${getDriverContextBadges(driver.name, driver.pos, driver.team)}
        <div class="row-points">${driver.points}<small>pts</small></div>
        <button class="fav-btn ${isFav ? "active" : ""}" onclick='event.stopPropagation(); setFavoriteDriver(${JSON.stringify(driver.name)}, ${JSON.stringify(driver.team)}, ${JSON.stringify(driver.number)}, ${JSON.stringify(String(driver.points))}, ${JSON.stringify(driver.colorClass)}, ${JSON.stringify(driver.image)}, ${JSON.stringify(driver.pos)})'>★</button>
      </div>
    </div>
  `;
}
...
function teamRow(team) {
  const favorite = getFavorite();
  const isFav = favorite.type === "team" && favorite.name === team.team;
  const isFavoriteDriverTeam = favorite.type === "driver" && favorite.team === team.team;
  const rowClass = [team.pos === 1 ? "leader" : "", isFav ? "favorite-row" : "", isFavoriteDriverTeam ? "teammate-row" : ""].join(" ").trim();

  return `
    <div class="standing-row ${rowClass}" onclick="openTeamDetail(${JSON.stringify(team.team)}, ${JSON.stringify(team.drivers)}, ${JSON.stringify(String(team.points))}, ${JSON.stringify(team.colorClass)}, ${JSON.stringify(team.pos)})">
      <div class="row-left">
        <div class="row-pos-wrap">
          <div class="row-pos">${team.pos}</div>
          ${renderDeltaBadge(state.standingsDelta.teams?.[team.team] || 0)}
        </div>
        <div class="row-stripe ${team.colorClass}"></div>
        <div class="row-info">
          <div class="row-name" style="color: var(--${team.colorClass});">${team.team}</div>
          <div class="row-team">
            <span>${team.drivers}</span>
            <img class="team-logo" src="${getTeamLogo(team.team)}" alt="${team.team}" onerror="this.style.display='none'">
          </div>
        </div>
      </div>
      <div class="row-badges">
        ${getTeamContextBadges(team.team, team.pos)}
        <div class="row-points">${team.points}<small>pts</small></div>
        <button class="fav-btn ${isFav ? "active" : ""}" onclick='event.stopPropagation(); setFavoriteTeam(${JSON.stringify(team.team)}, ${JSON.stringify(team.drivers)}, ${JSON.stringify(String(team.points))}, ${JSON.stringify(team.colorClass)}, ${JSON.stringify(team.pos)})'>★</button>
      </div>
    </div>
  `;
}
  ...
  return `
    <div class="calendar-event-card">
      <div class="calendar-event-top">
        <div>
          <div class="calendar-event-title">${event.type === "race" ? `R${event.round} · ${event.title}` : event.title}</div>
          <div class="calendar-event-sub">${event.venue} · ${event.location}</div>
        </div>
        <div class="calendar-event-right">
          ${formatCalendarDateRange(event.start, event.end)}<br>
          ${getCalendarStatusLabel(event.status, event.type)}
        </div>
      </div>

      <div class="calendar-event-tags">
        <span class="tag general">${event.type === "testing" ? "Testing" : "Carrera"}</span>
...
async function showCalendar(force = false) {
  ...
  contentEl().innerHTML = renderLoadingCard(
    "Calendario",
    "Cargando calendario oficial 2026 y separando próximas y completadas…",
    true
  );
...
async function showRaceMode() {
  ...
  contentEl().innerHTML = renderLoadingCard(
    "Modo carrera",
    "Preparando la lectura del próximo GP con predicción, top 10 y estrategia…",
    true
  );
...
if (!Array.isArray(safeJsonParse(localStorage.getItem("racecontrolPredictionHistory"), []))) {
  localStorage.setItem("racecontrolPredictionHistory", JSON.stringify([]));
}
...
window.saveSelectedRace = saveSelectedRace;