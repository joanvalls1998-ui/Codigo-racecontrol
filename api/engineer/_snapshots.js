...
function buildSnapshotFileName(snapshotId) {
  return `${SNAPSHOT_PREFIX}_${snapshotId}.json`;
}
...
function createDefaultIndex() {
  return { version: 1, updated_at: nowIso(), sessions: {}, latest_useful: null };
}

export async function getSnapshotIndex() {
  ensureSnapshotsDir();
  try {
    if (!fs.existsSync(INDEX_FILE)) return createDefaultIndex();
  ...
  } catch { return createDefaultIndex(); }
}
...
function buildSnapshotPayload({ snapshotId, params, payload, source = "runtime", status = "ready" }) {
  return {
    ...
    status,
    freshness: { status, generated_at: nowIso(), source, retries: 0, last_error: null },
    key: {
      year: params.year,
...
export async function markSessionSnapshotState({ year, meetingKey, session, status, reason = "", retries = 0, driversTotal = 0, driversReady = 0, nextRetryAt = null }) {
  ...
  index.sessions[ref] = {
    year, meeting_key: meetingKey, session_key: session.session_key,
    session_type: session.type_key, session_label: session.type_label,
    date_start: session.date_start || "", date_end: session.date_end || "",
    status, reason, retries, drivers_total: driversTotal, drivers_ready: driversReady,
    session_end_ms: Number.isFinite(sessionEndMs) ? sessionEndMs : null,
    generated_at: nowIso(), next_retry_at: nextRetryAt
  };
...
export function evaluateSessionEligibility(session = {}, nowMs = Date.now()) {
  const sessionEndMs = estimateSessionEndMs(session);
  if (!Number.isFinite(sessionEndMs)) return { eligible: false, reason: "SESSION_END_UNKNOWN", session_end_ms: null, ready_at_ms: null };
  const readyAt = sessionEndMs + GENERATION_MARGIN_MS;
  if (nowMs < readyAt) return { eligible: false, reason: "WAITING_TPLUS20", session_end_ms: sessionEndMs, ready_at_ms: readyAt };
  return { eligible: true, reason: "ELIGIBLE", session_end_ms: sessionEndMs, ready_at_ms: readyAt };
}

export function getRetryPolicy() {
  return { delay_ms: RETRY_DELAY_MS, max_retries: MAX_RETRIES, generation_margin_ms: GENERATION_MARGIN_MS };
}

export async function registerLatestUsefulMeeting({ year, meeting, sessions = [] }) {
  const index = await getSnapshotIndex();
  index.latest_useful = {
    year, meeting_key: meeting.meeting_key, gp_label: meeting.gp_label, round: meeting.round,
    updated_at: nowIso(), sessions_ready: sessions.filter(item => item.status === "ready").map(item => item.session_key)
  };
  index.updated_at = nowIso();
  await writeSnapshotIndex(index);
}
...
export function summarizeMeetingReadiness(index = {}, meetings = []) {
  ...
  const readiness = meetings.map(meeting => {
    ...
    return { meeting_key: meeting.meeting_key, gp_label: meeting.gp_label, total_sessions: rows.length, ready_sessions: ready, has_ready_snapshots: ready > 0 };
  });
  const latestUseful = readiness.filter(item => item.has_ready_snapshots).sort((a, b) => {
  ...
  return { latest_useful: index?.latest_useful || latestUseful || null, readiness };
}

export { SNAPSHOT_INDEX_KEY };